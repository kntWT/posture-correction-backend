...
from cruds.user import get_user_by_token, get_user_by_basic
...
def basic_auth(db: Session = Depends(get_db), credentials: HTTPBasicCredentials = Security(basic_security)) -> User:
    user_basic = UserBasicAuth(name=credentials.username, password=credentials.password)
    user = get_user_by_basic(db, user_basic)
    if user is None:
